package com.gerenciadorDeEstoqueEFluxoDeCaixa.constantes;

public class ConstantesMenuEstoque {
	
	public static final int CADASTRAR = 1;
	public static final int EDITAR = 2;
	public static final int LISTAGEM = 3;
	public static final int REMOVER = 4;
	public static final int ATIVAR_NOTA_FICAL = 5;
	public static final int LISTAGEM_VENDAS = 6;
	public static final int DETALHES_VENDA = 7;
	public static final int VOLTAR = 8;
	
}
